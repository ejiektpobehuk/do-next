...
use crate::jira::types::{
    Attachment, Comment, FieldMeta, FieldSchema, Issue, ProjectInfo, SearchResponse, StatusCategory,
    StatusInfo, Transition, TransitionsResponse,
};
    ...
    pub async fn get_all_statuses(&self) -> Result<Vec<StatusInfo>> {
        ...
        for s in raw {
            if seen.insert(s.name.clone()) {
                let category = s
                    .status_category
                    .map_or(StatusCategory::Undefined, |c| StatusCategory::from_key(&c.key));
                out.push(StatusInfo {
                    name: s.name,
                    category,
                });
